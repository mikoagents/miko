---
name: atmiko-setup-endpoint
description: Configure the public webhook endpoint for Atmiko — ngrok, Cloudflare Tunnel, or custom URL.
---
...
Configures a public URL so Linear (and other integrations) can send webhooks to your Atmiko instance.

## Step 1: Check Existing Configuration

```bash
grep -E '^ATMIKO_BASE_URL=' ~/.atmiko/.env 2>/dev/null
```

If `ATMIKO_BASE_URL` is already set, inform the user:

> Webhook endpoint already configured: `<value>`
> To reconfigure, remove `ATMIKO_BASE_URL` from `~/.atmiko/.env` and re-run this skill.
...
> **How will you expose Atmiko to the internet for webhooks?**
>
...
Write the ngrok config so it can be started with `ngrok start atmiko`:
...
#### 3d. Write ATMIKO_BASE_URL

```bash
printf 'ATMIKO_BASE_URL=https://%s\n' "<domain>" >> ~/.atmiko/.env
```
...
> To start ngrok, run: `ngrok start atmiko`
> Run this in a separate terminal before starting Atmiko.

### Option 2: Cloudflare Tunnel

Ask the user for:
- Their Cloudflare Tunnel token
- Their tunnel hostname (e.g., `atmiko.yourdomain.com`)
...
```bash
printf 'ATMIKO_BASE_URL=%s\n' "<url>" >> ~/.atmiko/.env
```

## Step 4: Write Common Config

Ensure these are present in `~/.atmiko/.env` (add only if missing):

```bash
grep -q '^ATMIKO_SERVER_PORT=' ~/.atmiko/.env 2>/dev/null || printf 'ATMIKO_SERVER_PORT=3456\n' >> ~/.atmiko/.env
grep -q '^LINEAR_DIRECT_WEBHOOKS=' ~/.atmiko/.env 2>/dev/null || printf 'LINEAR_DIRECT_WEBHOOKS=true\n' >> ~/.atmiko/.env
grep -q '^ATMIKO_HOST_EXTERNAL=' ~/.atmiko/.env 2>/dev/null || printf 'ATMIKO_HOST_EXTERNAL=true\n' >> ~/.atmiko/.env
```

`LINEAR_DIRECT_WEBHOOKS=true` enables direct Linear webhook signature verification. `ATMIKO_HOST_EXTERNAL=true` enables direct Slack and GitHub webhook signature verification. Both are required for self-hosted setups where webhooks come directly from the services (not forwarded through CYHOST).

## Completion

> ✓ Webhook endpoint configured: `<ATMIKO_BASE_URL>`
> ✓ Server port: 3456
