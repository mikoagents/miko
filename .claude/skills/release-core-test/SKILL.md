...
description: Invoke when dev-testing a Atmiko change that spans CYPACK (edgeworker + CLI) and CYHOST (Vercel-hosted GUI) and the hosted GUI needs to point at an unreleased `atmiko-core` from this repo. Publishes `atmiko-core` (and `claude-runner` if needed) as a `-test.N` prerelease under the npm `test` dist-tag so CYHOST can install it via `atmiko-core@test` without affecting the `latest` dist-tag or shipping a real release.
---

# Release Core (Test)

Publish ONLY the `atmiko-core` package (and `claude-runner` if needed) as a dev/test prerelease to npm under the `test` dist-tag. This is a developer convenience for iterating on `atmiko-core` without going through the full release flow.
...
It also does NOT publish any other workspace package (no `edge-worker`, no `atmiko` CLI, no `linear-event-transport`, etc.). The only exception is `claude-runner`, which is republished if and only if its `workspace:*` reference would otherwise resolve to an unpublished version (see step 3 below).
...
`atmiko-core` depends on `claude-runner` via `workspace:*`. When `atmiko-core` is published, that `workspace:*` is rewritten to the version currently in `packages/claude-runner/package.json`. If that version isn't actually on npm yet (because `claude-runner` has local changes since its last publish), consumers installing `atmiko-core@test` will fail to resolve the dependency.
...
npm view "atmiko-claude-runner@${LOCAL_CR}" version 2>/dev/null
...
```bash
# Determine the next -test.N for claude-runner (same algorithm as atmiko-core below)
...
cd ../..
pnpm install   # refresh lockfile so atmiko-core picks up the new claude-runner
```

Otherwise, skip this step entirely.

### 4. Bump `atmiko-core` to the next `-test.N` prerelease version
...
npm view atmiko-core versions --json \
  | jq -r '.[]' \
...
### 5. Publish `atmiko-core` with the `test` dist-tag
...
- `npm install atmiko-core@test` will install this version.
- The `latest` dist-tag is **not** moved, so production installs (`npm install atmiko-core`) are unaffected.

### 6. Verify the dist-tags landed correctly

```bash
npm view atmiko-core dist-tags
```
...
```
Published atmiko-core@<version> under dist-tag `test`.

Install with:
  npm install atmiko-core@test
or pinned:
  npm install atmiko-core@<version>
```
...
- Prerelease versions (anything with a `-` suffix like `-test.0`) are never auto-installed by `npm install atmiko-core` without an explicit tag or version spec, so this is safe to run repeatedly.
- Run `pnpm install` after publishing `claude-runner` (if applicable) so the lockfile reflects the new published version before `atmiko-core` is built/published.
- `pnpm install` + `pnpm build` from root takes ~3 minutes combined on a clean tree — plan command timeouts accordingly.

## Examples

- "release core test" - Run a dev release of atmiko-core
