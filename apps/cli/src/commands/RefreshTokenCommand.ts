...
export class RefreshTokenCommand extends BaseCommand {
	async execute(_args: string[]): Promise<void> {
		...
		if (workspaceStatuses.length === 0) {
			this.logError(
				"No Linear workspaces configured. Run 'atmiko self-auth-linear' first.",
			);
			process.exit(1);
		}
		...
		for (const index of indicesToRefresh) {
			...
			const serverPort = process.env.ATMIKO_SERVER_PORT
				? parseInt(process.env.ATMIKO_SERVER_PORT, 10)
				: 3456;
