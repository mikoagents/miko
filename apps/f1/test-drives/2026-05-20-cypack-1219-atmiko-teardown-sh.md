# Test Drive: CYPACK-1219 — per-repo `atmiko-teardown.sh` auto-detection

**Date**: 2026-05-20
**Goal**: Validate the per-repo `atmiko-teardown.sh` feature end-to-end through F1, using the new `f1 terminate-issue` command to drive issues to a terminal state.
**Test Repo**: `/tmp/f1-td-<ts>-a` (single-repo F1 server, port 3602)
**F1 server**: `ATMIKO_PORT=3602 ATMIKO_REPO_PATH=/tmp/f1-td-<ts>-a bun run apps/f1/server.ts`

## Result

**All 8 scenarios passed.** Per-repo `atmiko-teardown.sh` fires end-to-end through the F1 framework on all three terminal actions (`completed` / `canceled` / `deleted`), with correct `cwd` and `LINEAR_ISSUE_IDENTIFIER`, runs before worktree removal, is non-blocking on failure, is skipped silently when absent, and is not triggered by `stop-session`.
...
```bash
# /tmp/f1-td-.../atmiko-setup.sh
mkdir -p /tmp/atmiko-hooks
printf 'identifier=%s\ncwd=%s\nhook=setup\nrepo=%s\n' \
  "$LINEAR_ISSUE_IDENTIFIER" "$PWD" "<repo-basename>" \
  > "/tmp/atmiko-hooks/setup-$LINEAR_ISSUE_IDENTIFIER-<repo>.txt"
```

```bash
# /tmp/f1-td-.../atmiko-teardown.sh
mkdir -p /tmp/atmiko-hooks
printf 'identifier=%s\ncwd=%s\nhook=teardown\nrepo=%s\n' \
  "$LINEAR_ISSUE_IDENTIFIER" "$PWD" "<repo-basename>" \
  > "/tmp/atmiko-hooks/teardown-$LINEAR_ISSUE_IDENTIFIER-<repo>.txt"
```
...
→ teardown-DEF-1-*.txt written with cwd=/private/tmp/atmiko-f1-.../worktrees/DEF-1
...
[GitService] ℹ️  Running repository teardown script: atmiko-teardown.sh
...
### 4. No `atmiko-teardown.sh` present → no teardown attempt, worktree still removed

Renamed `atmiko-teardown.sh` aside, then:

```
DEF-4: setup ran, no teardown script present
terminate-issue --action completed
→ /tmp/atmiko-hooks/ contains setup-DEF-4 but NO teardown-DEF-4
...
Replaced `atmiko-teardown.sh` with one that writes an "attempt" sentinel then `exit 1`:
...
Server log:
```
[GitService] ❌ Repository teardown script failed: Command failed: bash ".../atmiko-teardown.sh"
