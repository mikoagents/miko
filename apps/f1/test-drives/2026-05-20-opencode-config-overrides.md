...
- [x] `pnpm --filter atmiko-core test:run` passed: 10 files, 131 tests.
- [x] `pnpm --filter atmiko-opencode-runner test:run` passed: 3 files, 15 tests.
- [x] `pnpm --filter atmiko-edge-worker test:run` passed: 56 files, 640 passed, 1 skipped.
...
- [x] Worktree was created at `/var/folders/_r/fld8l71j7ts635hlb5vtgnb80000gn/T/atmiko-f1-1779295065599/worktrees/DEF-1`.
...
```bash
pnpm --filter atmiko-core test:run
```

Result: PASS. `Test Files 10 passed (10)`, `Tests 131 passed (131)`.

```bash
pnpm --filter atmiko-opencode-runner test:run
```

Result: PASS. `Test Files 3 passed (3)`, `Tests 15 passed (15)`.

```bash
pnpm --filter atmiko-edge-worker test:run
```
...
```bash
ATMIKO_PORT=3600 ATMIKO_REPO_PATH="/var/folders/_r/fld8l71j7ts635hlb5vtgnb80000gn/T/opencode/opencode-config-f1/repo" bun run server.ts > "/var/folders/_r/fld8l71j7ts635hlb5vtgnb80000gn/T/opencode/opencode-config-f1/server.log" 2>&1 &
```
...
```bash
ATMIKO_PORT=3600 ./f1 create-issue --title "OpenCode runner selection smoke" --description "Validate that F1 can launch a Atmiko OpenCode runner session. Respond briefly with F1_OPENCODE_OK and do not modify files.\n\n[agent=opencode]"
```
...
```bash
ATMIKO_PORT=3600 ./f1 view-session --session-id session-1
```
...
```text
[GitService] Creating git worktree at /var/folders/_r/fld8l71j7ts635hlb5vtgnb80000gn/T/atmiko-f1-1779295065599/worktrees/DEF-1 from local main
[EdgeWorker] Starting agent session for issue DEF-1
[OpenCodeRunner] Unsupported config entry skipped: permission:SendMessage: Unsupported Atmiko tool pattern for OpenCode
error: OpenCode exited with code 1: Error: Session not found
```

```bash
ATMIKO_PORT=3600 ./f1 stop-session --session-id session-1
ATMIKO_PORT=3600 ./f1 view-session --session-id session-1 --limit 10 --offset 0
```
