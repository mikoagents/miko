...
Worktree: `/Users/jappy/.atmiko/worktrees/opencode-runner-selection`

## Goal

Validate that Atmiko can route an issue to the OpenCode runner when the issue uses the `opencode` label and `[agent=opencode]` selector.
...
ATMIKO_PORT=3600 ATMIKO_REPO_PATH=/tmp/f1-test-drive-opencode-selection \
...
ATMIKO_PORT=3600 apps/f1/f1 view-session --session-id session-1 --limit 50 --offset 0
...
The blocker appears to be local OpenCode runtime state rather than Atmiko runner selection, because Atmiko reached the OpenCode runner and surfaced the OpenCode process failure cleanly.
