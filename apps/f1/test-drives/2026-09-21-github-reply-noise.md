...
Built the changed GitHub transport and EdgeWorker packages, then started the F1 server on port 3600 with a fresh temporary Cyrus home and the configured Cursor credential. Production webhook integrations were not loaded.
...
CYRUS_PORT=3600 apps/f1/f1 view-session --session-id session-1 --limit 2 --offset 4
...
The motivating example was [Viora-Mono #842](https://github.com/VioraOS/Viora-Mono/pull/842). A running agent posted its own completion comment, Cyrus posted the agent's final shipping-check answer, and a queued notification then repeated the already-resolved Codex review. These were separate publications, not repeated deliveries of one webhook.
...
- Both complete GitHub system prompts give Cyrus ownership of the final reply and restrict work to the triggering request.
