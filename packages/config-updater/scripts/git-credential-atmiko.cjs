#!/usr/bin/env node
/**
 * git-credential-atmiko — git credential helper for multi-org GitHub access.
 *
 * Self-contained Node script (no dependencies). Installed by Atmiko at
 * `<atmikoHome>/scripts/git-credential-atmiko.cjs` and wired into git via:
 *
 ...
 * segment) in `<atmikoHome>/github-tokens.json` — the per-installation
 * GitHub App tokens pushed by atmiko-hosted — and prints credentials for a
