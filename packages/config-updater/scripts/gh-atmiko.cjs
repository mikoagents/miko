#!/usr/bin/env node
/**
 * gh-atmiko — per-invocation GitHub token resolution for the `gh` CLI.
 *
 * Installed by Atmiko at `<atmikoHome>/scripts/gh-atmiko.cjs`; the droplet's
 ...
 * token for the org it actually targets, mirroring how the Atmiko git
 ...
 *      repository").
 * Then the token, from `<atmikoHome>/github-tokens.json` (pushed by
 * atmiko-hosted):
 *   3. The org-matched token.
 *   4. `ATMIKO_GH_TOKEN` from the session env (set to the session's primary
...
/** Non-expired tokens from the Atmiko token store file. */
function loadValidTokens() {
	const atmikoHome =
		process.env.ATMIKO_HOME || path.join(os.homedir(), ".atmiko");
	const tokensFile = path.join(atmikoHome, "github-tokens.json");
	let tokens = [];
	try {
...
function resolveToken(args) {
	...
	if (process.env.ATMIKO_GH_TOKEN) return process.env.ATMIKO_GH_TOKEN;
...
function main() {
	...
	// wrapper contract); set GH_TOKEN only when Atmiko resolved a token.
	...
	const ghBin = process.env.ATMIKO_GH_REAL_BIN || "/usr/bin/gh";
	const result = spawnSync(ghBin, args, { stdio: "inherit", env });
	if (result.error) {
		console.error(`gh-atmiko: failed to run ${ghBin}: ${result.error.message}`);
		process.exit(127);
	}
