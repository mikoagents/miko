...
/**
 * Behavioral tests for scripts/gh-atmiko.cjs — the per-invocation gh token
 * resolver. The real script is spawned with ATMIKO_GH_REAL_BIN pointing at a
 * stub that prints the GH_TOKEN / GITHUB_TOKEN it received, so assertions
 * observe exactly what env the real gh would see.
 */
const SCRIPT = join(__dirname, "..", "scripts", "gh-atmiko.cjs");

describe("gh-atmiko per-invocation token resolution", () => {
	let dir: string;
	...
	function runGhAtmiko(
		...
		const result = spawnSync("node", [SCRIPT, ...args], {
			...
			env: {
				...process.env,
				ATMIKO_HOME: atmikoHome,
				ATMIKO_GH_REAL_BIN: stubGh,
				GITHUB_TOKEN: "customer_github_token",
				GH_TOKEN: "customer_gh_token",
				ATMIKO_GH_TOKEN: undefined,
				...opts.env,
	...
	beforeEach(() => {
		dir = mkdtempSync(join(tmpdir(), "gh-atmiko-test-"));
		atmikoHome = join(dir, ".atmiko");
		mkdirSync(atmikoHome, { recursive: true });
...
`,
	...
	it("resolves the token from the cwd's origin remote org", () => {
		...
		const { stdout } = runGhAtmiko(["pr", "create"], { cwd: repo });
	...
	it("prefers an explicit --repo argument over the cwd remote", () => {
		...
		for (const args of flagForms) {
			const { stdout } = runGhAtmiko(args, { cwd: repo });
	...
	it("resolves scp-style remotes (git@github.com:Org/repo.git)", () => {
		...
		const { stdout } = runGhAtmiko(["issue", "list"], { cwd: repo });
	...
	it("uses a repo view positional target instead of cwd or the session token", () => {
		...
		for (const args of [
			...
			const result = runGhAtmiko(args, {
				cwd,
				env: { ATMIKO_GH_TOKEN: "ghs_org_a" },
			});
	...
	it("does not treat option values or unrelated command text as repository targets", () => {
		...
		for (const args of [
			...
			expect(runGhAtmiko(args, { cwd }).stdout, args.join(" ")).toContain(
				"GH_TOKEN=ghs_org_a",
			);
		}
	});
	...
	it("resolves clone and fork sources without reading their option values or git arguments as targets", () => {
		...
		expect(
			runGhAtmiko(["repo", "fork", "--", "--reference", "OrgB/local"], { cwd })
				.stdout,
	...
	it("honors GH_REPO before cwd but after an explicit repository", () => {
		...
		expect(runGhAtmiko(["pr", "list"], opts).stdout).toContain(
			"GH_TOKEN=ghs_org_b",
		);
		expect(runGhAtmiko(["repo", "view", "OrgA/repo-a"], opts).stdout).toContain(
			"GH_TOKEN=ghs_org_a",
		);
		expect(runGhAtmiko(["pr", "list", "-ROrgA/repo-a"], opts).stdout).toContain(
			"GH_TOKEN=ghs_org_a",
		);
	});

	it("falls back to ATMIKO_GH_TOKEN outside a repo", () => {
		saveTokens([
			{ organization: "OrgA", token: "ghs_org_a" },
			{ organization: "OrgB", token: "ghs_org_b" },
		]);

		const { stdout } = runGhAtmiko(["api", "/user"], {
			env: { ATMIKO_GH_TOKEN: "ghs_session_token" },
		});
	...
	it("strips customer tokens and sets nothing when unresolvable", () => {
		...
		// Two tokens, no repo context, no ATMIKO_GH_TOKEN → no resolution;
