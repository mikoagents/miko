...
/**
 * Options for creating Atmiko tools with session management capabilities
 */
export interface AtmikoToolsOptions {
	/**
...
/**
 * Create a standard MCP SDK server with Atmiko tools.
 */
export function createAtmikoToolsServer(
	linearClient: LinearClient,
	options: AtmikoToolsOptions = {},
): McpServer {
		...
		async ({ agentSessionId, message }) => {
			...
			if (options.onFeedbackDelivery) {
				try {
					...
					console.error("[AtmikoTools] Failed to deliver feedback:", error);
				}
			}
	...
	// atmiko-hosted). Omitted in CLI mode where there is no control plane.
