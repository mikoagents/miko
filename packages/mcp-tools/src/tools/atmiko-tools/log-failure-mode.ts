 ...
 *   - `sessionId` — atmiko internal session UUID, used as the dedup key
 ...
 *     identifier (Linear "ENG-76", GitHub PR "atmiko#1234", GitLab MR,
...
/**
 * HTTP client interface for posting to atmiko-hosted. Tests can substitute
...
export interface LogFailureModeOptions {
	 ...
	 * server (e.g. parentSessionId passed to `createAtmikoToolsServer`).
	 */
					...
					content: [
						{
							...
							text: JSON.stringify({
								success: false,
								error: `atmiko-hosted POST /api/failure-modes failed: ${result.status} ${result.error}`,
							}),
						},
					],
				};
